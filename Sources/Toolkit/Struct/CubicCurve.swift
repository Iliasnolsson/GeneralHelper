...
public extension CubicCurve {
    ...
    func cgPath() -> CGMutablePath {
        ...
        path.move(to: start)
        path.addCurve(to: end, control1: startTangent, control2: endTangent)
        return path
    }
    
    func length() -> CGFloat {
        let fromVertex = CurveVertex(.zero, start, startTangent)
        let toVertex = CurveVertex(endTangent, end, .zero)
        return fromVertex.distanceTo(toVertex, sampleCount: 35)
    }
        
    func point(atPercentage t: CGFloat) -> CGPoint {
        let fromVertex = CurveVertex(.zero, start, startTangent)
        let toVertex = CurveVertex(endTangent, end, .zero)
        return fromVertex.splitCurve(toVertex: toVertex, position: t).trimPoint.point
    }
    
    func removeLengthFromStart(forPecentage lengthPercentage: CGFloat) -> CubicCurve {
        let curveRecessedStart = point(atPercentage: lengthPercentage)
